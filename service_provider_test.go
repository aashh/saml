...
func TestSPKeyInfoStrippedEvenWithX509Cert(t *testing.T) {
	// Regression test for issue #10: KeyInfo is always stripped from signatures
	// before validation, so signature validation relies solely on metadata certs.
	// Previously, KeyInfo was only stripped when X509Certificate was absent.
	//
	// This test injects a bogus X509Certificate into the Signature's KeyInfo
	// and verifies that ParseResponse still succeeds — proving that KeyInfo is
